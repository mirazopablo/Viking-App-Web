...
import { BudgetItemsTableEditor } from './BudgetItemsTableEditor';
import { DiagnosticPointsDrawer } from './DiagnosticPointsDrawer';
...
export const BudgetEditor: React.FC<BudgetEditorProps> = ({ diagnosticPoints = [] }) => {
  const { register, watch, setValue, control, formState: { errors } } = useFormContext<BudgetFormSchemaType>();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-muted/40 rounded-xl border">
        ...
        {/* Diagnostic Points Drawer Trigger */}
        <DiagnosticPointsDrawer diagnosticPoints={diagnosticPoints} />
      </div>
      ...
      <Card className="border-border/60 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          {/* Read-Only Literal Representation of Client & Device (Smart Intake Style) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl border border-border/80 bg-card/60 space-y-3">
              <div className="flex items-center justify-between border-b pb-2">
                ...
                <span className="text-[10px] font-mono bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 font-semibold">
                  IDENTIFICADO
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">NOMBRE COMPLETO</span>
                  <span className="font-bold text-foreground block text-sm mt-0.5">
                    {clientName || 'Sin especificar'}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">DNI / CUIT</span>
                  <span className="font-bold font-mono text-foreground block mt-0.5">
                    {clientDni || 'No registrado'}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">TELÉFONO</span>
                  <span className="font-semibold font-mono text-foreground block mt-0.5">
                    {clientPhoneNumber || 'No registrado'}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">DIRECCIÓN</span>
                  <span className="font-semibold text-foreground block mt-0.5">
                    {clientAddress || 'No registrada'}
                  </span>
                </div>

                <div className="col-span-2 border-t pt-2">
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">EMAIL</span>
                  <span className="font-medium text-foreground block font-mono text-[11px] mt-0.5">
            ...
            <div className="p-4 rounded-xl border border-border/80 bg-card/60 space-y-3">
              <div className="flex items-center justify-between border-b pb-2">
                ...
                <span className="text-[10px] font-mono bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded border border-blue-500/20 font-semibold">
                  EN REPARACIÓN
                </span>
              </div>
              ...
              <div className="space-y-2 text-xs">
                <div>
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">MARCA Y MODELO</span>
                  <span className="font-extrabold text-foreground block text-sm mt-0.5 uppercase tracking-wide">
                    {deviceModel || 'Sin especificar'}
                  </span>
                </div>

                <div className="pt-2 border-t">
                  <span className="text-[10px] text-muted-foreground uppercase font-mono block">N° DE SERIE / IMEI</span>
                  <span className="font-bold font-mono text-foreground block mt-0.5">
                    {deviceSerialNumber || 'No registrado'}
                  </span>
                </div>
              </div>
            </div>
